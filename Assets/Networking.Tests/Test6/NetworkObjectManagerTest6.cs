...
{
    public class NetworkObjectManagerTest6 : NetworkObjectManagerTest
    {
        ...
        {
            var proxy = actor.Create<ResourceCreator>((creator) => creator.Create("Light"));
            ...
            proxy.SetLocalPosition(Random.onUnitSphere * RADIUS);
            var parent = actor.Create<PrimitiveCreator>((creator) => creator.Create(PrimitiveType.Sphere));
